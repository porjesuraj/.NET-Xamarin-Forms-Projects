...
## Day 10: 11/05/2021
### 1. <a href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/app-fundamentals/triggers"> Triggers</a>
...
+ Trigger can be assigned at page,app, control level
+ Types of Triggers
1. Property triggers
2. Data triggers
3. Event triggers
...
#### 1. Property triggers
+ Property Trigger occurs when Property on a  control is set to a particular value. 
1. How to set PT
+   In Control or Style element, In controls Triggers Collection , we can set a Trigger, using 
+ TargetType -  control type that the trigger applies to. 
+  Using Property and its Value for which  trigger is activated
+   Setter - a collection of setter element, applied to control when trigger is activated 
+ EnterActions and ExitActions - can be used for trigger with setters.
...
+ Data Trigger uses data binding to monitor another control to be activated based on set Value 
+ here Instead of the Property attribute in a property trigger, set the Binding attribute to monitor for the specified value.
```xml
...
#### 3. Event triggers
 + Event trigger occur when a control event occur
 + The EventTrigger element requires only an Event property, such as "Clicked" and a Eventhandler i.e  a reference to a class defined by local:ClassName
 * A trigger action implementation class should
  +  The class  implements TriggerAction<T> where T = control 
...
#### 4. Multi triggers
 + allows multiple trigger condition,All the conditions must be true before the Setters are triggered.
 +  it can be combination of Property , Data and event Trigger. 
```xml
...
1. It is second  way to implement changes when a trigger occurs i.e 
  +  by adding EnterActions and ExitActions collections to the trigger
  +   and specifying TriggerAction<T> implementations. 
2. The EnterActions and  ExitActions  : are  collection  used to define an IList of TriggerAction objects, invoked when trigger is met and no longer met. 
...
+  A StateTrigger triggers a VisualState change when the IsActive property changes value.

#### Adaptive trigger
+ An AdaptiveTrigger triggers a VisualState change when the window is a specified height or width.
+  This trigger has two bindable properties:
 * MinWindowHeight, MinWindowWidth (double )

#### Compare state trigger
+ The CompareStateTrigger triggers a VisualState change when a property is equal to a specific value. 
+ This trigger has two bindable properties: 
  * 1. Property, of type object, which indicates the property being compared by the trigger.
  * 2. Value, of type object, which indicates the value at which the VisualState should be applied.
...
+  This trigger has a single bindable property:
  *  Device (string): indicates the device platform on which the VisualState should be applied.

#### Orientation state trigger
+ The OrientationStateTrigger triggers a VisualState change when the orientation of the device changes.
+  This trigger has a single bindable property:
...
### 2. <a href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/app-fundamentals/gestures/"> Gestures</a>

 + Gesture recognizers can be used to detect user interaction with views in a Xamarin.Forms application.
 +  GestureRecognizer class supports tap, pinch, pan, swipe, and drag and drop gestures on View instances which doesnt have events like click  or user actions.
#### 1.  Tap gesture recognizer
+ The tap gesture is used for tap detection and is implemented with the TapGestureRecognizer class.
+ Use : To make a user interface element clickable
 1. step to create a tap gesture
 *  create a TapGestureRecognizer instance, 
 *  handle the Tapped event and
 *   add the new gesture recognizer to the GestureRecognizers collection on the user interface element.
+ Property: NumberOfTapsRequired (int value) indicates no of tap required for event to occur.
...
```
 ...
 2. we can use ICommand for tap gesture, for app using MVVM pattern.
```xml
<Image Source="tapped.jpg">
    <Image.GestureRecognizers>
        <TapGestureRecognizer
...
#### 2. pinch gesture
 + it  is implemented with the PinchGestureRecognizer class.
 +  The pinch gesture is used for performing interactive zoom 
 + A common scenario for the pinch gesture is to perform interactive zoom of an image at the pinch location, accomplished by scaling the content of the viewport. 
 1. step to create a pinch gesture
  +  create a PinchGestureRecognizer instance
...
#### 3. pan gesture
+ it  is implemented with the PanGestureRecognizer
+ The pan gesture is used for detecting the movement of fingers around the screen and applying that movement to content,
+ i.e if a content is larger than screen size, so to view it which are not in screen, we can swipe a particular direction and get that side hidden view. 
+ common scenario for the pan gesture is to horizontally and vertically pan an image, for image bigger than screen/ViewPort
+ to make a user interface element moveable with the pan gesture
1.  step to create a pan  gesture
 +  create a PanGestureRecognizer instance
 +   handle the PanUpdated event and 
 +   add the new gesture recognizer to the GestureRecognizers collection on the user interface element.
...
#### 4. swipe gesture
+ A swipe gesture occurs when a finger is moved across the screen in a horizontal or vertical direction
+  used to initiate navigation through content. 
1.  step to create a pan  gesture
 +  create a SwipeGestureRecognizer instance
 +   set the Direction property to a SwipeDirection enumeration value (Left, Right, Up, or Down)
 +    optionally set the Threshold property--> (minimum swipe distance that must be achieved for a swipe to be recognized)
 +     handle the Swiped event, and
 +      add the new gesture recognizer to the GestureRecognizers collection on the view.
 ```xml
<BoxView Color="Teal" ...>
...
+ a single SwipeGestureRecognizer can only recognize   swipes that
 *  occur on the same axis., i.e we can set Direction = left, right or up ,down
 + or create one for each direction, with same or different event to each.
 ```xml
  <SwipeGestureRecognizer Direction="Left,Right" Swiped="OnSwiped"/>
 ```


#### 5. drag and drop gesture 
+ A drag and drop gesture enables items, and their associated data packages, to be dragged from one onscreen location to another location using a continuous gesture, in same or different app.
+ The drag source :  element on which the drag gesture is initiated, can provide data to be transferred by populating a data package object
+ The drop target:   the element under the drag source, then processes the data package.
1.  step to create a drag and drop gesture
 + Enable drag on an element by adding a DragGestureRecognizer object to its GestureRecognizers collection
 + [optional] Build a data package(not for image and text control)
 + Enable drop on an element by adding a DropGestureRecognizer object its GestureRecognizers collection.
 + [optional] Handle the DropGestureRecognizer.DragOver event 
 + [optional] Process the data package to receive the dropped content. 


### Find all validation where we can use triggers

1.	Xamarin.Forms support four types of triggers:
+ 
1.	Property Trigger 
+ executed when a property on a control is set to a particular value.
* UseCase Example :  
    1. set the BackgroundColor property when the property IsFocused of the Entry element is true.
    2. to change the background color and scale of the entry when the text matches a specific string
2.	Data Trigger 
+  same as the property trigger but uses data binding.
* UseCase Example :  
1. To disable the button when the entry’s Text.Length property is 0 for any Form.
2. Changes a Label's TextColor based on whether its related Entry has text entered into it or not
3. the second Entry is enabled only if the first Entry has text.
3.	Event Trigger 
+  occurs when an event occurs on the control.
* UseCase Example :  
1. Check the type of data entered  in a Entry control, and if incorrect change property like background color.
2 . add a scale animation when a button is tapped
4.	Multi Trigger 
+ allows multiple trigger conditions to be set before an action occurs.
* UseCase Example :  
1. enable the button when both email and password Entry are filled in by the user
2. enable a button according to the entries matching with a specific username and password, i.e. regular expression for them. 


2.	The Xamarin.Forms  supports 5 types of Gestures: 
+
 1. Tap 
 +  The tap gesture is used for tap detection, we can make a user interface element clickable using it.
* UseCase Example :  
     1. attach tap gesture to Image element, to show different images on tap
    2. Attach tap gesture to label element, so Entry element is opened for editing.

2. Pinch
+ The pinch gesture is used for performing interactive zoom
*  UseCase Example :  
     1.  attached to an Image element, to zoom in and out 
...
+ The pan gesture is used for detecting the movement of fingers around the screen and applying that movement to content
* UseCase Example :  
   1. Attached to an image with size greater than ViewPort.
  

4.  Swipe
+ A swipe gesture occurs when a finger is moved across the screen, used to initiate navigation through content.
* UseCase Example :  
  1.  To swipe through a collection of images, like in Carousel Collection

5.  drag and drop 
+  drag and drop gesture enables items drag from one location to another.
* UseCase Example :  
    1. reorder items in a CollectionView
    


##  Day 11: 12/05/2021 Xamarin.Forms templates
+ Templates are reusable components
### 1. <a href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/app-fundamentals/templates/control-template"> Control templates</a>
0. ControlTemplate Class 
+ are Template that specifies a group of styles and effects for controls.
+ Inherit : Object ->ElementTemplate-> ControlTemplate
+ Methods: CreateContent()

1.  control templates enable you to define the visual structure of custom controls, and pages. 
+ it separate the UI for a custom control, or page, from the logic (values) that implements the control or page.
+ content can also be inserted into the templated custom control/page using ContentPresenter Property.


+ it can be defined as Resource at page,app level i nResource Dictonary, with
  *  x: Key attribute to identify in resource directory.
  *   RelativeSource markup extension :  to set its BindingContext to the runtime object instance to which the template will be applied, which is known as the templated parent
+ to consume a control template, we set ControlTemplate property to the control template object.
...
<!--Consume a ControlTemplate -->
    <StackLayout Margin="30">
        <controls:CardView BorderColor="DarkGray"
                           CardTitle="John Doe"
                           CardDescription="Lorem ipsu dolorsit amet, ..."
...
2. The TemplateBinding markup extension
+ it  binds a property of an element that is in a ControlTemplate to a public property that is defined by the templated custom control or templated page.
 + use a TemplateBinding, to  enable properties on the control to act as parameters to the template. 
 +  itenables the RelativeSource binding from the previous control template to be removed, and replaces the Binding expressions
 + Properties: Path, Mode, Converter,ConverterParameter, StringFormat. 
  + it should only be used within a ControlTemplate. 
   + setting either TemplateBinding or BindingContext is same.
```xml
...
```
3. Apply a ControlTemplate with a style
+ Control templates can also be applied within a  implicit or explicit style
...
```
4. A ContentPresenter
+ it  can be placed in a control template to mark where content to be displayed by the templated custom control or templated page will appear.
+ 
```xml
<!--control template with contentPresenter -->
...
```xml
<!--  TealTemplate assigned to the ControlTemplate property of a ContentPage derived page: -->
<controls:HeaderFooterPage xmlns:controls="clr-namespace:ControlTemplateDemos.Controls"                           
 ControlTemplate="{StaticResource TealTemplate}" HeaderText="MyApp"...>
...
```
5. GetTemplateChild method :  which returns the named element in the instantiated ControlTemplate visual tree, if found. Otherwise, it returns null.
6. Bind to a viewmodel
+ A ControlTemplate can data bind to a viewmodel, even when the ControlTemplate binds to the templated parent

### 2. <a href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/app-fundamentals/templates/data-templates/">Data templates </a>

#### 1. Data Template 
+  provide the ability to define the appearance of data on supported controls.it uses data binding to display data.
+ Use:  for displaying data from a collection of objects in a ListView.
+ a data template can be reused by defining it as a custom type, or  as  control-level, page-level, or application-level resource.
   *  based on which its scope is decided, highest is app level. 
   * but precendence is for lower in view hierarchy i.e control level DT. 
+ Data Template can be part of  or placed in following properties:
  * ListView( Header/Footer/GroupHeader - template)
  * ItemsView(ItemTemplate - inherited by ListView)
  * MultiPage (ItemTemplate )   

#### 2. Creating a Xamarin.Forms DataTemplate
+ Data templates can be created 
 1. inline i.e inside ListView 
 2.  from a custom type i.e separate C# file
 3.    Creating a DataTemplate as a Resource in Resource Dictionary

+ Property:
  1.  ListView.ItemTemplate property 
   + it manages The appearance of the data for each cell in the ListView ,by setting it to a DataTemplate. 
  2. ListView.ItemSource property
   + containing data object as a array   

+  number of techniques ,to set appearance of each cell in the ListView is defined by a DataTemplate are : 

  1. Creating an Inline DataTemplate.
    + created using a ListView.ItemSource containing data object as a array 
    + and ListView.ItemTemplate having DataTemplate using Binding markup to access object property. 
    + used if there's no need to reuse the data template elsewhere. 
    + The child of an inline DataTemplate must be of, or derive from, type Cell. example : here ViewCell
    
  ```xml
     <!-- an Inline DataTemplate -->
  ...
  2. Creating a DataTemplate with a custom Type.
   + The ListView.ItemTemplate property can also be set to a DataTemplate that's created from a cell type.
   + here we have c# code, containing custom cell for dataTemplate,  ie. reusable throughout the application. 
   and custom cell must derive from type ViewCell
   
  ```xml
    <ContentPage xmlns:local="clr-namespace:DataTemplates">
    <StackLayout Margin="20">
        ...
        <ListView Margin="0,20,0,0">
           <ListView.ItemsSource>
  ...
  ```
 3. Creating a DataTemplate as a Resource.
...
#### 3. Creating a Xamarin.Forms DataTemplateSelector
+ A DataTemplateSelector can be used to choose a DataTemplate at runtime based on the value of a data-bound property.
+  This enables multiple DataTemplates to be applied to the same type of object, to customize the appearance of particular objects 
+  Use: a ListView binding to a collection of objects, whose apperance changes at runtime based on DataTemplate selected.

* A data template selector is implemented by
  1. creating a class that inherits from DataTemplateSelector. 
  2. The OnSelectTemplate method is then overridden to return a particular DataTemplate
```c#
...
* Consuming a DataTemplateSelector in XAML 
  1. the PersonDataTemplateSelector can be instantiated by declaring it as a resource
```xml
<ContentPage xmlns:local="clr-namespace:Selector;assembly=Selector" x:Class="Selector.HomePage">
  ...
  ...
  <!--PersonDataTemplateSelector instance is consumed -->
  <ListView x:Name="listView" ItemTemplate="{StaticResource personDataTemplateSelector}" />
</ContentPage>
```

## Day 12 : 13/05/2021  <a href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/app-fundamentals/custom-renderer/"> Custom Renderers </a>

### 1. Custom Renderers
1. Need for Custom Renders: 
+ Xamarin.Forms user interfaces are rendered using the native controls of the target platform, so to retain the appropriate look and feel for each platform. 
...
2. Custom Renderers
+ Xamarin.Forms page, layout, and control is rendered differently on each platform, using a Renderer class that in turn creates a native control.
+ we can implement our own custom Renderer class.
+ as it provide a powerful approach for customizing the appearance and behavior of Xamarin.Forms controls

3. If Not Custom Renders, we need :
+ a two-step process that involves 
+ creating a custom control through subclassing, and then
+  consuming the custom control in place of the original control.
* here, it is not possible to take advantage of platform-specific enhancements and customizations

4. Use: 
 +  used for small styling changes 
 +   sophisticated platform-specific layout and behavior customization

5. Drawbacks:
+ for a simple control customization using  custom renderer is often a heavy-weight response, so use Effects here
+ Custom controls in XAML for Shared Projects will require every application project to be configured with the same assembly name.
+ custom renderers are required in each platform project when rendering a View or ViewCell element.
+ if not custom renderers same can be  accomplished by using the Device class, or the OnPlatform markup extension, to provide platform-specific values.

6. Creating a Custom Renderer Class, steps: 
  + 
  1. **Create a subclass of the renderer class** that renders the native control.
  2. Override the ((OnElementChanged method)) i.e  used to render the native control and and write logic to customize the control.
  3. Add an **ExportRenderer attribute** to the custom renderer class i.e used to register the custom renderer with Xamarin.Forms.

### 2. Renderer Base Classes and Native Controls
 + Every Xamarin.Forms control(page, layout, view, and cell.) has an accompanying renderer for each platform that creates an instance of a native control.
 + Renderer for Views,Cells named format is ControlNameRenderer in Xamarin.forms.
 + namespaces for platform-specific renderers:
 1. iOS – Xamarin.Forms.Platform.iOS
 2. Android – Xamarin.Forms.Platform.Android

 + For page: page -> Renderer class -> ios (native Control ) ->android (native Control )
  1. Content page
> ContentPage -> 	PageRenderer ->	 UIViewController ->	ViewGroup

 + For layout: layout -> Renderer -> ios ->android
  1. StackLayout,Grid,Relative,Absolute,Flex,Templated  
   > Layout->ViewRenderer ->	UIView ->	View

 + For Views: Views -> Renderer -> ios ->android  
  1. Label
  > Label	->LabelRenderer->	UILabel ->	TextView

### 3. Customizing an Entry
+ The Xamarin.Forms Entry control allows a single line of text to be edited
+ For Entry : 
> page -> EntryRenderer class -> ios:  UITextField (native Control ) ->android: EditText. (native Control )
 
+ step to create custom render for Entry control:
...
+ process for creating the custom renderer class on each platformis as follows :
  1. Create a subclass of the EntryRenderer class 
  2.  Override the OnElementChanged method 
  3.  Add an ExportRenderer attribute to the custom renderer class
 
!['custom-entry-render-structure'](custom-entry-render-structure.png) 
...
namespace CustomRenderer.iOS
{
    public class MyEntryRenderer : EntryRenderer
    {
        ...
        {
          // here ioS Render is instantiated
...
```
### 4. Customizing a ContentPage for  live camera stream i 
+ A ContentPage is a visual element that displays a single view and occupies most of the screen
+ ContentPage -->PageRenderer(Renderer) -->ios:UIViewController (native Control ) --> Android: ViewGroup (native Control ). 

+  thee process for creating custom renderer for a ContentPage on each platform is as follows:
 1. Create a Xamarin.Forms page.
  + an unaltered ContentPage added to the shared project
 ```xml
 <ContentPage xmlns="http://xamarin.com/schemas/2014/forms"
             xmlns:x="http://schemas.microsoft.com/winfx/2009/xaml"
...
+ Create a subclass of PageRenderer class, Overridde OnElementChanged method, Add ExportRenderer attribute to class.
+ this is done for both Android and iOS. 



### 5 .Customizing a Map Pin
+ Xamarin.Forms.Maps provides a cross-platform abstraction for displaying maps
+  Map -->MapRenderer(Renderer class) -->ios: MKMapView (native Control )--> Android: MapView.(native Control )
+  the process for creating custom renderer for a Map on each platform is as follows:
 1. Create a Xamarin.Forms Custom map.
 2. Consume the Custom Map.
```xml
  <ContentPage ...
			       xmlns:local="clr-namespace:CustomRenderer;assembly=CustomRenderer">
...
```
3. Create the custom renderer for the page on each platform.
+ Create a subclass of MapRenderer class, Overridde OnElementChanged method, Add ExportRenderer attribute to class.
+ this is done for both Android and iOS. 

### 6. Customizing a ListView
+  ListView is a view that displays a collection of data as a vertical list.
+ Render to create native view flow is
>  ListView -->ListViewRenderer(Renderer class) -->ios: UITableView(native Control ) --> Android:ListView (native Control ).
+  the process for creating custom renderer for a ListView on each platform is as follows:
 1. Creating the Custom ListView Control, by subclasing from ListView.
 2. Consume the custom control from Xamarin.Forms.
 ```xml
  <Grid>
 ...
 ```
 3. Create the custom renderer for the control on each platform.
  + Create a subclass of ListViewRenderer  class, Overridde OnElementChanged method, Add ExportRenderer attribute to class.
   + this is done for both Android and iOS. 

### 7.  Customizing a ViewCell
+  ViewCell is a cell that can be added to a ListView or TableView, which contains a developer-defined view.
+  Render to create native view flow is
>  ViewCell -->ViewCellRenderer(Renderer class) -->ios: UITableViewCell (native Control ) --> Android:View (native Control ).
+  the process for creating custom renderer for a ViewCell on each platform is as follows:
  1. Create a Xamarin.Forms custom cell, subclassing from ViewCell
  2. Consume the custom cell from Xamarin.Forms.
  3. Create the custom renderer for the cell on each platform. 
   +  Create a subclass ofViewCellRenderer  class, Overridde OnElementChanged method, Add ExportRenderer attribute to class.
   + this is done for both Android and iOS. 

### 8.  Customizing a WebView : how to create a custom renderer that extends the WebView to allow C# code to be invoked from JavaScript.
+ A Xamarin.Forms WebView is a view that displays web and HTML content in your app.
+   Render to create native view flow is
>  WebView -->WkWebViewRenderer(Renderer class) -->ios: WkWebView (native Control )--> Android:WebView (native Control ).
+  the process for creating custom renderer for a WebView on each platform is as follows:
 ...
 3. Create the custom renderer for the HybridWebView on each platform.
   +  Create a subclass of WkWebViewRenderer  class on iOS and WebViewRenderer class on Android , Overridde OnElementChanged method, Add ExportRenderer attribute to class.
   + this is done for both Android and iOS. 

### 9. Implementing a View :to create a custom renderer  to display a preview video stream from the device's camera.
+ View class is used to place layouts and controls on the screen., so custom user interface control should derive from it.
+  Render to create native view flow is
>  View -->ViewRenderer(Renderer class) -->ios: UIView (native Control )--> Android:View (native Control ).
+ the process for creating custom renderer for a View on each platform is as follows:
 1. Creating the Custom Control, subclassing View class
 2. Consuming the Custom Control, in XAML page
 3. Creating the Custom Renderer on each Platform, here process is different for iOS and Android. 


## Day 13 14/05/2021 <a href="https://docs.microsoft.com/en-us/xamarin/xamarin-forms/enterprise-application-patterns/mvvm">The Model-View-ViewModel Pattern  </a>

1. The Model-View-ViewModel (MVVM) pattern helps to cleanly separate the business logic , presentation logic and user interface (UI) of a application
2. Using the MVVM pattern, the UI of the app and the underlying presentation and business logic is separated into three separate classes 
 +  There are three core components in the MVVM pattern: 
 + 
  1. the model
   +  which encapsulates the app's business logic and data.
   +  responsible to encapsulate app data, like POCO class
  2. the view 
   +  which encapsulates the UI and UI logic;
   + responsible for defining the structure, layout and appearance of what the user sees on screen.
  3. the view model. 
   + which encapsulates presentation logic and state
   + implements properties properties and commands for which the view can data bind to.

!['mvvm'](mvvm.png) 

3. the ViewModel isolates the view from the model, and allows the model to evolve independently of the view.

4. Benifits:
 + improve code re-use opportunities 
 + The app UI can be redesigned without touching the code, if view is entirely in XAML 
 + Designers can focus on the view, while developers can work on the view model and model components.
 + can create unit tests for the view model and the model, without using the view. 

5. Multiple MVVM libraries for use  from Nuget package: 
 + Prism
 + MVVMLight
 + MVVMCross
 + FreshMVVM etc, 
 + can also develop custom MVVM (create structure of app where to write what code  )

6. How to select MVVM library for an app?
 + it depends on developer or client preference
  + proper experience.

7. MVVM Prism wit Unity(IOC container)
+ Prism is a framework for building loosely coupled, maintainable , and testable XAML applications for Xamarin.forms.
+ Started by microsoft prism 5
+ it supports multiple IOC containers like Autofac,Dryloc, (microsoft)Unity,Ninject etc.
+ comes with template pack as anvs extension
 * user can create a new prism project from the template. 

8.  Impelementing Prism with X.Forms
 + 2 options
   1. using Prism template from vs extension
   2. manually addd prism to X.forms app
   




####  The following sections discuss the responsibilities of each of the classes in the MVVM pattern.

1. View
 + The view is responsible for UI that the user sees on screen.
 + defined mostly in XAML, only UI logic for animation is in code behind C#. 
 + a view is typically a Page-derived or ContentView-derived class,can  also be represented by a data template.
 + A data template as a view does not have any code-behind, and is designed to bind to a specific view model type.
 ...
 * Execution of code on View model based on View changes
 + done by click, tap, Command property,behavior

2.  ViewModel
 + The view model implements properties and commands to which the view can data bind to,
 + i.e define the functionality to be offered by the UI, while view decides how to display them 
 +  it notifies the view of any state changes through change notification events.

 *  view model is also responsible for coordinating the view's interactions with any model classes that are required.
  + Each view model provides data from a model in a form that the view can easily consume i.e can do data conversion if required. 

 * for 2 way databinding between VM and V: 
   1. VM properties must raise the PropertyChanged event. 
   2.  View and models satisfy this requirement by implementing the INotifyPropertyChanged interface, and
   3.   raising the PropertyChanged event when a property is changed.

3. Model
 + Model classes are same as POCO classes. i.e non-visual classes that encapsulate the app's data.
 +  Examples : data transfer objects (DTOs), Plain Old CLR Objects (POCOs), POJO
 + they work with services, repositories.

####  Connecting View Models to Views

1.  aim is for the view to have a view model assigned to its BindingContext property. 
+ it can be done, using approaches falling into two categories:
   + 1. view first composition, and 
   + 2. view model first composition.

2.  view first composition 
 +  app is conceptually composed of  views that connect to the view models they depend on.
 +  benefit : it makes it easy to construct loosely coupled, unit testable apps 

3.  view model first composition.
 +  the app is conceptually composed of view models, with a service being responsible for locating the view for a view model
 + benifit: developer can focus on the logical non-UI structure of the app
 + drawback: a view model first composition complex and misaligned with the platform.

#### approaches to connecting view models to views

1. Creating a View Model Declaratively in XAML
...
    ...  
 ...
 ```
2. Creating a View Model Programmatically
 +  here needs to provide the view model with any required dependencies
```c#
public LoginView()  
{  
    InitializeComponent();  
...
+ example of viewmodel first 

4. Automatically Creating a View Model with a View Model Locator
 + A view model locator is a custom class that manages the instantiation of view models and their association to views 
 + using property AutoWireViewModel

#### Updating Views in Response to Changes in the Underlying View Model or Model
1. INotifyPropertyChanged interface 
 + Implementing this interface in a view model or model class allows the class to provide change notifications to any data-bound controls in the view when the underlying property value changes.

#### UI Interaction using Commands and Behaviors
 + In mobile apps, action/events are invoked mainly in response to user action like button click, calling a code implemented in code-behind using a handler. 
 + In MVVM pattern, the event handler is implemented by view model, so no handler code in code behind file
 + the way this can be done is using 
  1. Command
   +   provide a convenient way to represent actions that can be bound to controls in the UI. 
   +    controls can be declaratively connected to a command, and these controls will invoke the command when the user interacts with the control.
  2. Behaviors
   +  it allow controls to be declaratively connected to a command, also can invoke range of events raised by control.

#### Implementing Commands
   + View models typically expose command properties, for binding from the view
   + controls provide a Command property, which can be data bound to an ICommand object provided by the view model.
   + ICommand interface  methods
    * Execute, CanExecute, CanExecuteChanged

#### Invoking Commands from a View
 + it need a Command property set to action. optionally COmmandParameter can also be there.  
```xml
...
#### Implementing Behaviors
 +  In the context of MVVM, behaviors are a useful approach for connecting controls to commands.
 + EventToCommandBehavior property is particularly useful for attaching a command to a control that doesn't support commands
...
## Udemy Course 1
+ Using VS2019 Xamarin Android template, created Xamarin Platform specific controls 
+ here we to Set a Layout using SetContentType , where layout was a xml page where using ToolBOx we can Drag and drop controls to out page and further add things to control using android: syntax. 
+ we have MainActivity.cs as our only code back file, where we set our layout and Handle events
+ different Controls we learn during the course where :
 1. How to Define and Style Buttons
 2. How to add Texts and Text-Boxes
 3. How to Define and Style Decision Making Elements like Button, ToggleButton,CheckBox,RadioButtons 
 4. How to add List view items with List Adapter
 5. Add Differnet Layouts to your App like Linear,Relative layout
 6. Add and Style your items w/o write codes using xml designer toolbox

+ is ANdroid, our project when build is JIT compiled so at time of running the application IL is converted into native code
+ in case of iOS , our project when builded is AOT(Ahead of time ) compiled so IL is converted to ioS native code at build time  and when application runs, its native code
## Udemy Course 2
...
* We use x:Name attribute to assign an identifier to an element. 
* This will generate a private field that we can access in code-behind or XAML:
 ...
 * Example : ContentPage, StackLayout

6. Data Binding
 + for single binding we use Source and Path Property for each Attribute to be set.
```xml
<Label	Text=“{Binding	
...
```      	
 * If we we have multiple bindings to an object,we set BindingContext to the control or Layout level
 When BindingContext applied to a container element, it’ll be inherited by all its children.
```xml
...
8. Dealing with Device Differences
* To differentiate between various devices in code-behind, we can use switch case with Device.RuntimePlatform as condition statement.
* * in Xaml we can use OnPlatform syntax in Property Element Syntax.



## Asynchronous Programming using async and await keyword in .NET
1.	If a programmer wants to run some part of the code separately from the main thread execution, he will make use of this Asynchronous Programming.,
 this  can be done in  C# using async and await keywords.
+ The keyword async is prefixed to a method to tell  that, this methods runs an asynchronous task           
+ The keyword await means waiting for an event to happen.
         
2.	Asynchronous in .NET uses only three return types:
+ Task  :  it does not return a value and represents only a single operation.
+ Task<T> : it  is a return value of Type T and represents only a single operation.
+ Void : does not return a value.

3.	Different Methods of Task class used in async Programming:
...
+ Dispose() : Releases all resources used by the current instance of the Task
...
4.	Asynchronous programming increases the responsiveness and throughput of the application, should use the Task-based asynchronous pattern.
Use  of Async: 
+ If you want to do input/output operations like downloading a large resource or reading it from the network, 
we use await keyword on an async method, which returns a Task or Task<T>
...
1. Fast renderers 
+ using this we get direct access to native UI, rather than Render  to that Navite UI element,
+ it increases our app performance as 50% work of creating a render is reduced here
+ for using this , we need to include in both our platform spefiic project, i.e in AppDelegate and MainActivity.cs

```c#
// include before Xamarin.Forms
            global::Xamarin.Forms.Forms.SetFlags("FastRenderers_Experimental");

```

2. creating a IValueConvertor class
+ to pass to the View a certion format done in Convert method
+ to return back to Model  format it understand in ConverBack method
+ example of DateCOnvertor 
```c#
...
+ At the time of Rendering , All Components of UI in a neste layout  ,are not shown in UI,  so they are removed from the visual tree
+ in case of a COntent Page having StackLayout -> Grid -> Controls
+ here we can use layout compression to increase the performance of the app
...
4. Syncronising data between SQLlite and Azure Database
+ we need plugin : Microsoft.Azure.Mobile.Client.SQLiteStore
+ here we have to set a store i.e database path and a SyncContext 

5. storing images on cloud i.e  Azure 
+ we need to create a Storage Service, called : Storage accound, blob,file,table ,queue
+ we need WindowsAzure.Storage Plugin for Cloud storage, for all project
* Why not SQlite : -> as SQL is not optimized to store Images, but we can still store images as string in SQLlite 


## Following are some key features  of .NET MAUI:

1. Splash Screen
+  .NET MAUI now has a single place in which a developer can describe a splash screen

2. Raw Assets
+ Developers can more easily add assets to a project and reference them directly, by annotate it as a MauiAsset in the properties.

3. BlazorWebView:
+  This let developers host a Blazor web application right in a .NET MAUI application
+ to leverage seamless native platform features and UI controls.

4. new Single Project functionality for selecting a target platform and device when running apps, simplifying the startup process.

5. In the new target debug selector, you will select the platform you are targeting first then, you will be given the list of devices you can run your .NET MAUI application on.

6. To get started with .NET MAUI, developers can use the command-line tool maui-check (installed with dotnet tool install -g Redth.Net.Maui.Check)

7. New .NET Hot Reload enables you to make live edits to your .NET MAUI app’s source code while it is running

8. preview version of Telerik UI for MAUI provides the .NET developer community with professionally designed, feature rich UI components to start their cross-platform development. 
9. component vendors are also planning .NET MAUI support such as Syncfusion, DevExpress, and GrialKit.
...
11. Multiple Design Patterns Support
+ Model-View-View-Model (MVVM) and XAML are first-class features in .NET MAUI. 
+ Developers can now implement the Model-View-Update (MVU) pattern.
+ MVU promotes a one-way flow of data and state management along with a code-first development experience, which updates the UI by applying necessary changes.
